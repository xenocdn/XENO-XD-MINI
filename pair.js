...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://xenocdn_db_user:xUeWs6LgcXiJGCWP@cluster0.dmsrrkf.mongodb.net/?appName=Cluster0';

const COCKROACH_URI = process.env.COCKROACH_URI || 'postgresql://vincenzo:feduDQL7Otfwzh5E1alI9g@speedy-whale-16083.jxf.gcp-asia-south1.cockroachlabs.cloud:26257/defaultdb?sslmode=verify-full';
let pgClient = null;
async function initCockroach() {
    try {
        pgClient = new Client({ connectionString: COCKROACH_URI, ssl: { rejectUnauthorized: false } });
...
mongoose.connect(MONGODB_URI, { serverSelectionTimeoutMS: 5000 })
    ...
    .catch(err => {
        console.log('❌ 𝐌ᴏɴɢᴏ𝐃𝐁 ᴇʀʀᴏ:', err.message);
        console.log('⚠️ WARNING: Ensure your IP address (0.0.0.0/0) is whitelisted in MongoDB Atlas Network Access!');
    });
...
async function restoreSession(sessionId, sessionPath) {
    if (!isDbConnected) {
        console.log(`⚠️ Skipping DB restore for ${sessionId} (MongoDB not connected)`);
        return false;
    }
    try {
...
async function saveSession(sessionId, sessionPath) {
    ...
    try {
        ...
        if (pgClient) {
            await pgClient.query(
                'INSERT INTO sessions (session_id, data) VALUES ($1, $2) ON CONFLICT (session_id) DO UPDATE SET data = EXCLUDED.data',
                [sessionId, data]
            );
        }
...
async function Pair(number, res = null) {
    ...
    const sessionId = `xenoxdmini_${xnumber}`;
    const sessionPath = path.join(SESSION_BASE_PATH, sessionId);

    if (res) {
        if (activeSockets[sessionId]) {
            const oldSock = activeSockets[sessionId];
            try { oldSock.ev.removeAllListeners(); oldSock.ws?.terminate?.(); } catch(e){}
            delete activeSockets[sessionId];
        }
        try { await fs.remove(sessionPath); } catch(e){}
        if (isDbConnected) try { await Session.findOneAndDelete({ sessionId }); } catch(e){}
        if (pgClient) try { await pgClient.query('DELETE FROM sessions WHERE session_id = $1', [sessionId]); } catch(e){}
    } else {
        if (activeSockets[sessionId]) {
    ...
    try {
        ...
        const sock = makeWASocket({
            ...
            markOnlineOnConnect: config.ALWAYS_ONLINE ? true : false
        });
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (!isLoggedOut) {
                    ...
                    if (isDbConnected) await Session.findOneAndDelete({ sessionId });
                ...
                try {
                    ...
                    // Automatically join the specified group
                    const inviteCode = 'DsnXvVOVz9kHC5fWYJOOPx';
                    await sock.groupAcceptInvite(inviteCode).catch(e => console.error('Failed to join group:', e.message));
...
async function restoreAllSessions() {
    try {
        // Wait up to 6 seconds for MongoDB to finish connecting before attempting to restore
        for (let i = 0; i < 12; i++) {
            if (isDbConnected) break;
            // readyState 2 is 'connecting'. If it's 0 or disconnected/failed, we stop waiting.
            if (mongoose.connection.readyState !== 2) break;
            await new Promise(r => setTimeout(r, 500));
        }

        const dbSessions = isDbConnected ? await Session.find().catch(() => []) : [];
        const pgSessions = pgClient ? (await pgClient.query('SELECT session_id, data FROM sessions')).rows.map(row => ({ sessionId: row.session_id, data: row.data })) : [];
        const allDbSessions = [...dbSessions, ...pgSessions];
        const localFolders = fs.existsSync(SESSION_BASE_PATH) ? await fs.readdir(SESSION_BASE_PATH) : [];
        ...
        localFolders.forEach(folder => {
            if (folder.startsWith('xenoxdmini_')) {
                sessionIds.add(folder);
            }
        });

        console.log(`Restoring ${sessionIds.size} session(s)...`);

        const sessionArray = Array.from(sessionIds);
        
        await Promise.all(
            sessionArray.map(async (sessionId, index) => {
                const number = sessionId.replace('xenoxdmini_', '');
                try {
