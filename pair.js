...
// ---------------- CONFIG ----------------
const BOT_NAME_FANCY = '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg',
  ...
  BOT_NAME: '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'ᴅᴛᴢ ᴅᴜʟᴀ💔',
  IMAGE_PATH: 'https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg',
  BOT_FOOTER: '> *©ᴘᴏᴡᴇʀᴅ ʙʏ 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚*',
  BUTTON_IMAGES: { ALIVE: 'https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg' }
};
// ---------------- MONGO SETUP (Load Splitting) ----------------

const MONGO_URI_1 = process.env.MONGO_URI_1 || 'mongodb+srv://seya1:seya1freedula@seya.txln08n.mongodb.net/?appName=seya'; // Sessions සහ Numbers සඳහා
const MONGO_URI_2 = process.env.MONGO_URI_2 || 'mongodb+srv://seya2:seya2dula@seya2.jvf7hhz.mongodb.net/?appName=seya2'; // Configs, Admins, Newsletters සඳහා
const MONGO_DB_1 = process.env.MONGO_DB_1 || 'SEYA_XMDV';
const MONGO_DB_2 = process.env.MONGO_DB_2 || 'SEYA_XMD';

let mongoClient1, mongoClient2, mongoDB1, mongoDB2;
let sessionsCol, numbersCol; // DB 1 සඳහා
let adminsCol, newsletterCol, configsCol, newsletterReactsCol; // DB 2 සඳහා
...
async function initMongo() {
  try {
    // Database දෙකම දැනටමත් සම්බන්ධ වී ඇත්නම් නැවත සම්බන්ධ නොවේ
    if (mongoClient1 && mongoClient1.topology && mongoClient1.topology.isConnected() &&
        mongoClient2 && mongoClient2.topology && mongoClient2.topology.isConnected()) {
      return;
    }
  } catch(e){}

  try {
    // --- 1. Primary DB (Core Data) ---
    if (!mongoClient1 || !mongoClient1.topology || !mongoClient1.topology.isConnected()) {
        mongoClient1 = new MongoClient(MONGO_URI_1, { useNewUrlParser: true, useUnifiedTopology: true });
        await mongoClient1.connect();
        mongoDB1 = mongoClient1.db(MONGO_DB_1);
        
        sessionsCol = mongoDB1.collection('sessions');
        numbersCol = mongoDB1.collection('numbers');

        await sessionsCol.createIndex({ number: 1 }, { unique: true });
        await numbersCol.createIndex({ number: 1 }, { unique: true });
        console.log('✅ DB 1 (Core: Sessions & Numbers) Connected!');
    }

    // --- 2. Secondary DB (Settings & Data) ---
    if (!mongoClient2 || !mongoClient2.topology || !mongoClient2.topology.isConnected()) {
        mongoClient2 = new MongoClient(MONGO_URI_2, { useNewUrlParser: true, useUnifiedTopology: true });
        await mongoClient2.connect();
        mongoDB2 = mongoClient2.db(MONGO_DB_2);
        
        adminsCol = mongoDB2.collection('admins');
        newsletterCol = mongoDB2.collection('newsletter_list');
        configsCol = mongoDB2.collection('configs');
        newsletterReactsCol = mongoDB2.collection('newsletter_reacts');

        await newsletterCol.createIndex({ jid: 1 }, { unique: true });
        await newsletterReactsCol.createIndex({ jid: 1 }, { unique: true });
        await configsCol.createIndex({ number: 1 }, { unique: true });
        console.log('✅ DB 2 (Data: Configs, Admins, Newsletters) Connected!');
    }
...
async function saveNewsletterReaction(jid, messageId, emoji, sessionNumber) {
  try {
    ...
    // මෙතන mongoDB වෙනුවට mongoDB2 භාවිතා කළ යුතුය
    if (!mongoDB2) {
        await initMongo();
        if (!mongoDB2) return;
    }
    const col = mongoDB2.collection('newsletter_reactions_log');
...
async function getReactConfigForJid(jid) {
  try {
    await initMongo();
    if (!newsletterReactsCol) return null;
...
// ========== END WORK TYPE RESTRICTIONS ==========
    ...
    let botName = cfg.botName || '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚';
...
🔒 _This menu is restricted to the bot owner only._` 
    ...
    const botName = currentConfig.botName || '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚'; // Default name fallback
...
case 'gpt': {
  try {
    ...
    if (!q) {
      await socket.sendMessage(sender, { 
        text: `*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙿𝚕𝚎𝚊𝚜𝚎 𝙿𝚛𝚘𝚟𝚒𝚍𝚎 𝙰 𝚀𝚞𝚎𝚜𝚝𝚒𝚘𝚗 !*`,
        buttons: [
    ...
    const metaQuote = {
      ...
      message: { 
        contactMessage: { 
          displayName: botName, 
          vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${botName};;;;\nFN:${botName}\nORG:🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚\nTEL;type=CELL;type=VOICE;waid=94700000000:+94 70 000 0000\nEND:VCARD` 
        } 
      }
    };

    await socket.sendMessage(sender, { react: { text: '🧠', key: msg.key } });
    await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙰𝙸 𝙸𝚜 𝚃𝚑𝚒𝚗𝚔𝚒𝚗𝚐...*', quoted: metaQuote });
    ...
    await socket.sendMessage(sender, {
      text: `👸 *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐀𝐈 𝐂𝐇𝐀𝐓* 🧠\n\n${aiReply}\n\n*✨ 𝙼𝚊𝚍𝚎 𝙱𝚢 🩵 ᴅᴛᴢ  🩵*`,
      footer: `🤖 ${botName}`,
      buttons: [
    ...
    await socket.sendMessage(sender, { 
      text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙰𝙿𝙸 𝙴𝚛𝚛𝚘𝚛 𝚃𝚛𝚢 𝙰𝚐𝚊𝚒𝚗 𝙻𝚊𝚝𝚎𝚛 !*',
      buttons: [
        { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📄 𝐌𝐀𝐈𝙽 𝐌𝐄𝐍𝐔' }, type: 1 }
      ]
    }, { quoted: msg });
...
case 'upload': {
    try {
        ...
        const botLogo = 'https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg'; // REPLACE WITH YOUR LOGO URL
        ...
        const msgParams = {
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        body: {
                            text: `╭━━❮ *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 UPLOADER* ❯━━╮
│
│ 📁 *Type:* ${mediaType}
...
> 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐕𝟐 🤍`
                        },
                        footer: {
                            text: "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚"
                        },
                        header: {
                            title: "Media Uploaded Successfully",
                            subtitle: "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚",
                            hasMediaAttachment: false
                        },
                        contextInfo: {
                            externalAdReply: {
                                title: "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 UPLOADER",
 ...
 case 'weather':
    try {
        ...
        const messages = {
            ...
            weather: (data) => `
* 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 ᴡᴇᴀᴛʜᴇʀ ʀᴇᴘᴏʀᴛ *
...
╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯`
                        ...
                        footer: proto.Message.InteractiveMessage.Footer.create({
                            text: "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 • Secure Connection"
                        }),
...
END:VCARD`
        ...
        // 8. වීඩියෝව යැවීම (🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 Style Caption සමඟ)
        await socket.sendMessage(sender, {
...
*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ ${botName}*`,
            contextInfo: {
                externalAdReply: {
                    ...
                    body: "ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚",
...
case 'save': {
  try {
    ...
    if (!quotedMsg) {
      return await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙿𝚕𝚎𝚊𝚜𝚎 𝚁𝚎𝚙𝚕𝚢 𝚃𝚘 𝙰 𝚂𝚝𝚊𝚝𝚞𝚜 !*' }, { quoted: msg });
    }
    ...
    if (quotedMsg.imageMessage || quotedMsg.videoMessage || quotedMsg.audioMessage || quotedMsg.documentMessage || quotedMsg.stickerMessage) {
      ...
      if (!media || !media.buffer) {
        return await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙵𝚊𝚒𝚕𝚎𝚍 𝚃𝚘 𝙳𝚘𝚠𝚗𝚕𝚘𝚊𝚍 𝙼𝚎𝚍𝚒𝚊 !*' }, { quoted: msg });
      }

      let captionText = media.caption || '';
      const botCaption = `\n\n👸 *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 📥\n\n*✨ 𝙼𝚊𝚍𝚎 𝙱𝚢 🩵 ᴅᴛᴢ  🩵*`;
      ...
      if (quotedMsg.imageMessage) {
        ...
        const fname = media.fileName || `🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚Saved.${(await FileType.fromBuffer(media.buffer))?.ext || 'bin'}`;
      ...
      await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 💫\n\n*✅ 𝙳𝚘𝚠𝚗𝚕𝚘𝚊𝚍𝚎𝚍 𝚂𝚞𝚌𝚌𝚎𝚜𝚜𝚏𝚞𝚕𝚕𝚢 !*' }, { quoted: msg });

    } else if (quotedMsg.conversation || quotedMsg.extendedTextMessage) {
      const text = quotedMsg.conversation || quotedMsg.extendedTextMessage.text;
      await socket.sendMessage(saveChat, { text: `👸 *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 📥\n\n${text}\n\n*✨ 𝙼𝚊𝚍𝚎 𝙱𝚢 🩵 ᴅᴛᴢ  🩵*` });
      ...
      if (typeof socket.copyNForward === 'function') {
        try {
          ...
          await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙴𝚛𝚛𝚘𝚛 𝙵𝚘𝚛𝚠𝚊𝚛𝚍𝚒𝚗𝚐 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 !*' }, { quoted: msg });
        }
      } else {
        await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝚄𝚗𝚜𝚞𝚙𝚙𝚘𝚛𝚝𝚎𝚍 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚃𝚢𝚙𝚎 !*' }, { quoted: msg });
      }
    }

  } catch (error) {
    console.error('❌ Save error:', error);
    await socket.sendMessage(sender, { text: '*👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 : 𝙵𝚊𝚒𝚕𝚎𝚍 𝚃𝚘 𝚂𝚊𝚟𝚎 𝚂𝚝𝚊𝚝𝚞𝚜 !*' }, { quoted: msg });
  }
  break;
}
case 'alive': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚'; // Default fancy name
    const logo = cfg.logo || config.RCD_IMAGE_PATH;
...
${frame}`, edit: key });
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚";
    const logo = cfg.logo || config.RCD_IMAGE_PATH;
...
┃
╰━━〔 *${config.OWNER_NAME || '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚'}* 〕━━┈⊷
...
END:VCARD` 
    ...
    await socket.sendMessage(sender, {
      image: imagePayload,
      caption: text,
      footer: `*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚*`,
      buttons: buttons,
      headerType: 4,
      contextInfo: {
        externalAdReply: {
  ...
  try {
    ...
    try {
        loadMsg = await socket.sendMessage(safeSender, { 
            text: `🔄 *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐒𝐘𝐒𝐓𝐄𝐌 𝐁𝐎𝐎𝐓...*` 
        }, { quoted: msg });
    } catch (e) {
    ...
    await socket.sendMessage(safeSender, { 
        text: `📡 *Connecting to 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 Server...*
[⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜] 0%`, 
        edit: loadKey 
    });
    ...
    const botName = "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚";
    const defaultLogo = "https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg";
...
╰──────────────────────`;
    ...
    const metaQuote = {
      key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚_STATUS" },
      message: { 
        contactMessage: { 
          displayName: botName, 
          vcard: `BEGIN:VCARD
VERSION:3.0
N:XMD;🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚;;
FN:${botName}
ORG:🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 Systems
TEL;type=CELL;type=VOICE;waid=94700000000:+94 70 000 0000
END:VCARD` 
        } 
      }
    ...
    await socket.sendMessage(safeSender, {
      image: imagePayload,
      caption: text,
      footer: `👸 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 𝐒𝐘𝐒𝐓𝐄𝐌`,
      contextInfo: {
        externalAdReply: {
          title: `${botName} 𝐌𝐨𝐧𝐢𝐭𝐨𝐫`,
          body: `👸 𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚`,
          previewType: "PHOTO",
    ...
    try {
        await socket.sendMessage(sender, { 
            text: '❌ *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 Error:* An unexpected system error occurred.' 
        }, { quoted: msg });
    } catch (e) {}
  }
  break;
        ...
        const interactiveMsg = generateWAMessageFromContent(sender, {
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: {
                        body: { text: artBody },
                        footer: { text: `Download by 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚` }, // Branding
                        header: {
                ...
                if (type) {
                    ...
                    try {
                        ...
                        const contextInfo = {
                            externalAdReply: {
                                title: videoInfo.title,
                                body: "Download by 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚",
                                thumbnailUrl: videoInfo.thumbnail,
                                sourceUrl: videoInfo.url,
                                mediaType: 1,
                                renderLargerThumbnail: true
...
case 'download': {
  ...
  try {
    ...
    const title = userCfg.botName || '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚';
    
    // 1. GENERATE RANDOM LOGO (Add your URLs here)
    const logos = [
        "https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg", 
        "https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg",
...
`.trim();
    ...
    await socket.sendMessage(sender, {
      ...
      caption: text,
      footer: "🚀 ᴘᴏᴡᴇʀᴇᴅ ʙʏ 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚",
      buttons: buttons,
      contextInfo: {
        externalAdReply: {
...
case 'creative': {
  ...
  try {
    ...
    // Random Logo Logic
    const logos = [config.LOGO, "https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg"]; // Add more
    const randomLogo = logos[Math.floor(Math.random() * logos.length)] || logos[0];
...
case 'system': {
  ...
  try {
    ...
    // Random Logo Logic
    const logos = [config.LOGO, "https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg"]; 
    const randomLogo = logos[Math.floor(Math.random() * logos.length)] || logos[0];
...
case 'dp': {
    ...
    try {
        ...
        const botName = cfg.botName || "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚"; // Default Artful Name
...
case 'owner': {
  try {
    ...
    const botName = '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚';
    const ownerImage = 'https://i.ibb.co/TqxNwDG8/tourl-1771764560884.jpg';
    ...
    const buttonParams = [
      {
        ...
        buttonParamsJson: JSON.stringify({
          ...
          url: `https://wa.me/${ownerNumber}?text=Hello ${ownerName}, I need assistance with 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 Bot.`
        })
      },
      {
        name: "cta_url",
      ...
      viewOnceMessage: {
        message: {
          ...
          interactiveMessage: {
            ...
            header: {
              title: "",
              subtitle: "🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚 Support",
              hasMediaAttachment: true,
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> *🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚*',
            buttons: buttons,
             headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname 🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚`' }, { quoted: shonux });
  }
...
async function deleteSessionAndCleanup(number, socketInstance) {
  const sanitized = number.replace(/[^0-9]/g, '');
  try {
    if (keepAliveTimers.has(sanitized)) {
        clearInterval(keepAliveTimers.get(sanitized));
        keepAliveTimers.delete(sanitized);
    }
    if (reconnectTimers.has(sanitized)) {
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (isLoggedOut) {
        ...
        if (keepAliveTimers.has(sanitizedNum)) {
            clearInterval(keepAliveTimers.get(sanitizedNum));
            keepAliveTimers.delete(sanitizedNum);
        }
        activeSockets.delete(sanitizedNum);
        socketCreationTime.delete(sanitizedNum);
        const attemptReconnect = async () => {
            try { 
                ...
                reconnectTimers.set(sanitizedNum, setTimeout(attemptReconnect, 10000));
            }
        };
        attemptReconnect();
      }
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    if (!socket.authState.creds.registered) {
      let retries = config.MAX_RETRIES;
      let code;
      let customId = `ABC${makeid(5).toUpperCase()}`;
     
      while (retries > 0) {
        try { 
          await delay(2000); 
          // Using customId as a secondary parameter if the Baileys version supports it, 
          // otherwise falling back to standard pairing.
          code = await socket.requestPairingCode(sanitizedNumber, customId); 
          if (code) break; 
        } catch (error) { 
          console.error(`Pairing attempt failed (${retries} retries left):`, error);
          retries--; 
          await delay(3000); 
        }
      }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          if (keepAliveTimers.has(sanitizedNumber)) {
            clearInterval(keepAliveTimers.get(sanitizedNumber));
          }
          keepAliveTimers.set(sanitizedNumber, setInterval(async () => {
              if (!activeSockets.has(sanitizedNumber)) {
                  clearInterval(keepAliveTimers.get(sanitizedNumber));
                  keepAliveTimers.delete(sanitizedNumber);
                  return;
              }
              try {
                  await socket.sendPresenceUpdate('available');
              } catch (err) {
                  console.log('Keep-alive failed for', sanitizedNumber);
                  deleteSessionAndCleanup(sanitizedNumber, socket);
                  const attemptReconnect = async () => {
                      try {
                          await delay(3000);
                          const mockRes = { headersSent:false, send:() => {}, status: () => mockRes }; 
                          ...
                          console.error('Reconnect attempt failed', e); 
                          reconnectTimers.set(sanitizedNumber, setTimeout(attemptReconnect, 10000));
                      }
                  };
                  attemptReconnect();
              }
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '🦚𝐐ᴜᴇᴇɴ 𝐒ᴇʏᴀ 𝐗ᴍᴅ 𝐕3🦚', activesession: activeSockets.size });
});
